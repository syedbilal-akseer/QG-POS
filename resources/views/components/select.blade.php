@props(['disabled' => false, 'name' => null, 'multiple' => false, 'options' => [], 'placeholder' => 'Select an option'])

@php
    // Fall back to wire:model value so Alpine.js @this.get() works when name is not passed explicitly
    $resolvedName = $name ?? ($attributes->get('wire:model') ?? $attributes->get('wire:model.live') ?? '');
    $isInvalid = $resolvedName ? $errors->has($resolvedName) : false;

    // Normalize options: accept both ['key' => 'Label'] and [['value'=>'key','label'=>'Label']] formats
    $normalizedOptions = [];
    foreach ($options as $key => $value) {
        if (is_array($value) && array_key_exists('value', $value)) {
            $normalizedOptions[] = $value; // Already in {value, label} format
        } else {
...
<div x-data="{
    name: '{{ $resolvedName }}',
    options: {{ json_encode(array_values($normalizedOptions)) }},
    multiple: {{ $multiple ? 'true' : 'false' }},
    selected: @this.get('{{ $resolvedName }}') ? @this.get('{{ $resolvedName }}') : {{ $multiple ? json_encode(old($resolvedName, [])) : json_encode(old($resolvedName, null)) }},
    isOpen: false,
    search: '',
    ...
    selectOption(value) {
        ...
        @this.set('{{ $resolvedName }}', this.multiple ? this.selected : this.selected); // Ensure it's a single value for single selection
...
>
    <!-- Hidden Input -->
    <input type="hidden" @if($resolvedName) name="{{ $resolvedName }}" @endif x-bind:value="multiple ? selected.join(',') : selected" />
