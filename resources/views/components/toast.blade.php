<div x-cloak x-data="{ showSuccess: false, showError: false, showWarning: false, message: '', timeout: 7000 }" aria-live="assertive"
     ...
     @toast-warning.window="showWarning = true; message = $event.detail; setTimeout(() => { showWarning = false; }, timeout);"
     x-init="
         @if (session('success'))
             showSuccess = true;
         ...
         @if (session('warning'))
             showWarning = true;
             message = {!! json_encode((string) session('warning')) !!};
             setTimeout(() => { showWarning = false; }, timeout);
         @endif
     ">

    <div x-show="showSuccess || showError || showWarning" x-transition:enter="transform ease-out duration-300 transition"
        ...
        <!-- Warning Toast (e.g. duplicate-PDF re-upload) -->
        <div x-show="showWarning" class="p-4 flex items-start">
            <div class="flex-shrink-0">
                <svg class="h-6 w-6 text-yellow-500 dark:text-yellow-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="ml-3 flex-1 pt-0.5">
                <p class="text-sm font-medium text-gray-900 dark:text-gray-100">Heads-up</p>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400" x-text="message"></p>
            </div>
            <div class="ml-4 flex-shrink-0">
