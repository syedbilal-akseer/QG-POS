...
<x-app-layout>
    ...
    <script>
        // Send the CSRF token on every AJAX request, freshly read from the
        // <meta> tag each time so the keep-alive component's token rotation
        // is picked up automatically. Capturing it once at load time (the
        // previous behaviour) caused intermittent 419 errors after the token
        // rotated mid-session.
        $.ajaxSetup({
            ...
            $('#sendToOracleBtn').click(function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr, textStatus, errorThrown) {
                        ...
                        if (xhr.status === 419) {
                            // Silently refresh the CSRF token (via keep-alive)
                            // and ask the user to retry. No page reload — the
                            // batch insert into Oracle may have already partly
                            // succeeded, so blowing away the page is worse than
                            // letting the user click again.
                            try { fetch('{{ route('keep-alive') }}', { credentials: 'same-origin' }); } catch (e) {}
                            alert('Page token refreshed. Please click "Send to Oracle" once more.');
                            return;
                        }
                        if (xhr.status === 0) {
