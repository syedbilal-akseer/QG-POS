<x-app-layout>
        ...
        <div class="w-full max-w-none ">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div class="flex-1 max-w-sm mx-4">
                    <form action="{{ route('invoices.index') }}" method="GET" target="_blank" rel="noopener" class="relative group">
                        ...
                        @if(request('search'))
                            <a href="{{ route('invoices.index') }}" target="_self" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-red-500 transition-colors">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-0 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-8 overflow-hidden rounded-2xl border border-blue-200 dark:border-blue-900 overflow-hidden shadow-sm">
                        <div class="px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/5 dark:to-indigo-900/5 border-b border-blue-200 dark:border-blue-900 flex items-center">
                            <i class="fas fa-search-location text-blue-600 dark:text-blue-400 mr-3"></i>
                            <h3 class="text-sm font-bold text-blue-900 dark:text-blue-300 uppercase tracking-widest">
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-900/50">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">File Name</th>
                                        <th class="px-6 py-3 text-left text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Size</th>
                                        <th class="px-6 py-3 text-left text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Modified</th>
                                        <th class="px-6 py-3 text-right text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Action</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/40 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            @endphp
                                            <a href="{{ route('invoices.preview-file', ['path' => $file['path']]) }}" target="_blank"
                    ...
                    <div id="queueStatusSection" class="mb-6 hidden bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                            <div class="flex-1 w-full">
                                <div class="flex justify-between items-center mb-1">
                                    <span class="text-sm font-semibold text-blue-800 dark:text-blue-300">WhatsApp Sending Queue</span>
                                    <span id="queuePercent" class="text-xs font-bold text-blue-700 dark:text-blue-400">0%</span>
                                </div>
                                <div class="w-full bg-blue-200 dark:bg-blue-800 rounded-full h-2.5">
                                    <div id="queueBar" class="bg-blue-600 h-2.5 rounded-full transition-all duration-500" style="width: 0%"></div>
                                </div>
                            ...
                            <div class="flex gap-2">
                                <button onclick="location.reload()" class="p-2 text-blue-600 hover:bg-blue-100 rounded-full transition-colors" title="Refresh Page">
                                    <i class="fas fa-sync-alt"></i>
                                </button>
                            </div>
                        </div>
                        <div id="queueFailures" class="mt-3 text-xs border-t border-blue-100 dark:border-blue-800 pt-2 hidden">
                        ...
                        <form method="GET" action="{{ route('invoices.index') }}" class="flex flex-wrap items-end gap-3">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400 ml-auto">
                                @if($filterFrom && $filterTo && $filterFrom === $filterTo)
                                    Showing uploads from <strong class="text-gray-700 dark:text-gray-200">{{ \Carbon\Carbon::parse($filterFrom)->format('d M Y') }}</strong>
                                @elseif($filterFrom || $filterTo)
                                    Showing
                                    <strong class="text-gray-700 dark:text-gray-200">{{ $filterFrom ? \Carbon\Carbon::parse($filterFrom)->format('d M Y') : 'beginning' }}</strong>
                                    to
                                    <strong class="text-gray-700 dark:text-gray-200">{{ $filterTo ? \Carbon\Carbon::parse($filterTo)->format('d M Y') : 'today' }}</strong>
                                @else
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                        ...
                        <div class="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg border border-orange-200 dark:border-orange-800 flex flex-col justify-between">
                    ...
                    @if($unsentOnly ?? false)
                        <div class="mb-4 px-4 py-3 rounded-md bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 text-sm text-orange-800 dark:text-orange-200 flex items-center justify-between">
                            <div>
                                <i class="fas fa-paper-plane mr-2"></i>
                                <strong>Unsent / Pending filter active</strong> — showing only completed invoices that have NOT been sent on WhatsApp.
                            </div>
                            <a href="{{ route('invoices.index', request()->except(['unsent_only','page'])) }}"
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <table class="min-w-full bg-white dark:bg-gray-800">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th class="px-3 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider w-64">
                                        Customer
                                    </th>
                                    <th class="px-3 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider">
                                        Invoice Details
                                    </th>
                                    <th class="px-3 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider">
                                        Status
                            ...
                            @php
                                ...
                                $groupedInvoices = $invoices->groupBy(function ($i) {
                            ...
                            @forelse($groupedInvoices as $groupDate => $groupRows)
                                @php
                                    ...
                                    foreach ($groupRows as $row) {
                                        ...
                                        if (($row->whatsapp_status ?? null) === 'sent')   $whatsappSent++;
                                        if (($row->whatsapp_status ?? null) === 'failed') $whatsappFailed++;
                                ...
                                <tbody x-data="{ open: false }"
                                    ...
                                    {{-- Accordion header — use the same gray-800/gray-100
                                         ...
                                         and dark themes. --}}
                                    <tr class="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
                                        <td colspan="6" class="px-6 py-3 text-xs font-bold text-gray-800 dark:text-gray-100 uppercase tracking-wider">
                                            <div class="flex items-center justify-between gap-3 flex-wrap">
                                                {{-- Left half: chevron + date label + counts. Clicking this part toggles the accordion. --}}
                                                <div @click="open = !open" class="flex items-center cursor-pointer flex-1 min-w-0">
                                                    ...
                                                    <i class="far fa-calendar-alt mr-2 text-primary-600 dark:text-primary-400 shrink-0"></i>
                                                    <span class="mr-3 shrink-0">
                                                        @if($groupDate === '__no_date__')
                                                            No upload date
                                                        @elseif($groupDate === $today)
                                                            Today &middot; {{ \Carbon\Carbon::parse($groupDate)->format('d M Y') }}
                                                        @elseif($groupDate === $yesterday)
                                                            Yesterday &middot; {{ \Carbon\Carbon::parse($groupDate)->format('d M Y') }}
                                                        @else
                                                            {{ \Carbon\Carbon::parse($groupDate)->format('l, d M Y') }}
                                                        @endif
                                                    </span>
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary-600 text-white normal-case tracking-normal shrink-0">
                                                        {{ $groupRows->count() }} {{ Str::plural('invoice', $groupRows->count()) }}
                                                    </span>
                                                    ...
                                                    <div class="flex items-center gap-1.5 ml-3 normal-case tracking-normal">
                                                        @if($statusCounts['completed'] > 0)
                                                            <span style="background-color: #16a34a; color: #ffffff;"
                                                                ...
                                                                <i class="fas fa-check-circle mr-1"></i>{{ $statusCounts['completed'] }} Completed
                                                            </span>
                                                        @endif
                                                        @if($statusCounts['processing'] > 0)
                                                            <span style="background-color: #eab308; color: #ffffff;"
                                                                  class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold">
                                                                <i class="fas fa-spinner mr-1"></i>{{ $statusCounts['processing'] }} Processing
                                                            </span>
                                                        @endif
                                                        @if($statusCounts['failed'] > 0)
                                                            <span style="background-color: #dc2626; color: #ffffff;"
                                                                  class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold">
                                                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $statusCounts['failed'] }} Failed
                                                            </span>
                                                        @endif
                                                        @if($statusCounts['pending'] > 0)
                                                            <span style="background-color: #6b7280; color: #ffffff;"
                                                                  class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold">
                                                                <i class="fas fa-clock mr-1"></i>{{ $statusCounts['pending'] }} Pending
                                                            </span>
                                                        @endif
                                                        @if($whatsappSent > 0)
                                                            <span style="background-color: #059669; color: #ffffff;"
                                                                  class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium">
                                                                <i class="fab fa-whatsapp mr-1"></i>{{ $whatsappSent }} Sent
                                                            </span>
                                                        @endif
                                                        @if($whatsappFailed > 0)
                                                            <span style="background-color: #b91c1c; color: #ffffff;"
                                                                  class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium">
                                                                <i class="fab fa-whatsapp mr-1"></i>{{ $whatsappFailed }} Failed
                                                            </span>
                                                        @endif
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2 shrink-0">
                                                    @if(!empty($unsentIds))
                                                        <button type="button"
                                                            ...
                                                            <i class="fas fa-paper-plane mr-1"></i>Send {{ count($unsentIds) }}
                                                        </button>
                                                    @endif
                                                    <span @click="open = !open" class="text-[10px] font-medium text-gray-600 dark:text-gray-300 normal-case tracking-normal cursor-pointer">
                                    ...
                                    @foreach($groupRows as $invoice)
                                    <tr x-show="open" x-cloak
                                        class="hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                        <td class="px-3 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    <div class="h-10 w-10 rounded-full bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 flex items-center justify-center text-gray-700 dark:text-gray-200 font-bold">
    ...
    <div class="relative inline-block">
            ...
            <div
                ...
                <div class="flex flex-wrap gap-1">
                    @foreach($numbers->slice(2) as $num)
                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-semibold bg-primary-50 dark:bg-primary-900/30 text-primary-800 dark:text-primary-200 border border-primary-100 dark:border-primary-800">
                            #{{ $num }}
                        </span>
                    @endforeach
                </div>
        ...
        @endif
                                                ...
                                                @endphp
                                                <div class="text-xs font-bold text-blue-600 dark:text-blue-400 mt-1">
                                        ...
                                        <td class="px-3 py-4 whitespace-nowrap">
                                            ...
                                            @switch($invoice->processing_status)
                                                @case('completed')
                                                    ...
                                                    @break
                                                @case('processing')
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-yellow-500 text-white">
                                        ...
                                        <td class="px-3 py-4 whitespace-nowrap">
                                            <div class="flex flex-col space-y-1">
                                                <div class="flex items-center">
                                                    <span id="phone-{{ $invoice->id }}" class="text-sm text-gray-900 dark:text-gray-100 mr-2">
                                                        {{ $invoice->customer_phone ?: 'No Phone' }}
                                                    </span>
                                                    <button onclick="editPhoneNumber({{ $invoice->id }}, '{{ $invoice->customer_phone }}')"
                                                ...
                                                @if($invoice->whatsapp_status === 'sent')
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-600 text-white">
                                                        <i class="fas fa-check mr-1"></i>Sent {{ $invoice->whatsapp_sent_at->diffForHumans() }}
                                                    </span>
                                                @elseif($invoice->whatsapp_status === 'failed')
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-600 text-white" title="{{ $invoice->whatsapp_error }}">
            ...
            <div class="py-1">
                ...
                @if($invoice->processing_status === 'completed' && $invoice->pdf_path)
                    ...
                    <button
                        ...
                        <i class="fab fa-whatsapp w-5 text-green-600"></i>
                        Send WhatsApp
                    </button>
    ...
    <!-- Progress Modal -->
    <div id="progressModal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4">
                                ...
                                <div id="progressDetail" class="mt-4 text-xs text-gray-500 dark:text-gray-400 max-h-40 overflow-y-auto">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" id="closeProgressBtn" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white dark:bg-gray-800 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm hidden" onclick="location.reload()">
                        Close & Refresh
                    </button>
                </div>
            </div>
