<x-app-layout>
    ...
    @php
        $isEdit = (bool) $bill;
        $action = $isEdit ? route('vendor-bills-2.update', $bill) : route('vendor-bills-2.store');
    @endphp
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        {{ $isEdit
                            ? 'Update the details below; on save the bill goes back for approval.'
                            : 'Fill in the bill details and attach supporting documents.' }}
                    </p>
                </div>
                <a href="{{ $isEdit ? route('vendor-bills-2.show', $bill) : route('vendor-bills-2.index') }}"
            ...
            @if($isEdit && $bill->status === 'rejected')
                <div class="mb-6 px-4 py-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm text-red-800 dark:text-red-200">
                    ...
                    Update details and attachments then resubmit.
                </div>
            @endif
            ...
            <form method="POST" action="{{ $action }}" enctype="multipart/form-data"
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="text-sm">
                            <a href="#" class="text-orange-600 hover:text-orange-700 font-medium">
                                <i class="fas fa-plus mr-1"></i> Create new vendor
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tax Amount (PKR)</label>
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 font-semibold">Rs</span>
                                ...
                                @foreach($bill->attachments as $att)
                                    <li class="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('vendor-bills-2.attachment', $att) }}" target="_blank"
