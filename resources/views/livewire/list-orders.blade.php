...
@endpush
<div>
    {{-- Stats panel — driven by ListOrders::getStatsData() which uses
         getFilteredTableQuery(), so every figure here reflects the currently
         selected Filament filters (salesperson, pushed-by, customer, date
    ...
    <div class="mb-6 grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
            ...
            <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">in current filter</div>
        </div>

        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
            <div class="text-xs font-semibold uppercase tracking-wider text-yellow-700 dark:text-yellow-300">Pending</div>
            <div class="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{{ number_format($stats['pending']) }}</div>
            <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">awaiting Oracle push</div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <div class="text-xs font-semibold uppercase tracking-wider text-primary-700 dark:text-primary-300">TAT &le; 24h</div>
                @if($stats['delayed'] > 0)
                    @if($this->isDelayedReport())
                        <button type="button" wire:click="clearTatReport"
                                class="text-[11px] font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
                            <i class="fas fa-times mr-1"></i>Clear
                        </button>
                    @else
                        <button type="button" wire:click="showTatReport"
            ...
            <div class="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $stats['tatPct'] }}%</div>
            <div class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                {{ number_format($stats['pushedOnTime']) }} of {{ number_format($stats['pushed']) }} pushed within 24h
                @if($stats['delayed'] > 0)
                    · <span class="text-red-600 dark:text-red-400 font-semibold">{{ number_format($stats['delayed']) }} delayed</span>
                @endif
            </div>
        </div>
    </div>
    ...
    @if($this->isDelayedReport())
        <div class="mb-4 px-4 py-3 rounded-md bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm text-red-800 dark:text-red-200 flex items-center justify-between">
            <div>
                <i class="fas fa-stopwatch mr-2"></i>
                <strong>TAT Report active</strong> — showing {{ number_format($stats['delayed']) }} order(s) that crossed the 24h SLA.
            </div>
            <button type="button" wire:click="clearTatReport"
    ...
    @if ($order)
        <x-modal name="order_detail" :show="true" focusable maxWidth="screen">
            ...
            <div class="px-4 md:px-6 pb-4 md:pb-6">
                <div class="mt-6 space-y-6">
                    ...
                    {{-- Cancellation banner — only shown when the order has been
                         cancelled. Surfaces the reason, who cancelled it, and
                         when, so the audit context is immediately visible to
                         anyone opening the View modal. --}}
                    @if ($order->order_status === \App\Enums\OrderStatusEnum::CANCELED)
                        <div class="flex items-start gap-3 px-4 py-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                            <svg class="w-5 h-5 text-red-600 dark:text-red-400 shrink-0 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                            </svg>
                            <div class="flex-1">
                                <span class="text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wide">Order Cancelled</span>
                                <p class="text-sm text-red-900 dark:text-red-100 mt-0.5">
                    ...
                    {{-- Order Items Table — Excel-style frozen first column.
                         All sticky/min-width rules are inline styles so no other
                         stylesheet (Filament, modal animation, Tailwind preflight)
                         can override them. --}}
                    <div class="mt-4 border border-gray-200 dark:border-neutral-700 rounded-lg overflow-hidden">
                        <div style="max-height: 220px; overflow: auto;">
                            <table style="border-collapse: separate; border-spacing: 0;" class="min-w-full">
                                <thead>
                                    <tr>
                                        <th style="position: sticky; top: 0; left: 0; z-index: 30; min-width: 140px;" class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-r border-b border-gray-200 dark:border-neutral-700">Item Code</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 500px;" class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Description</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 60px;"  class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Qty</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 60px;"  class="px-3 py-2 text-center text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">UOM</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 90px;"  class="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Price</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 90px;"  class="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Cap Price</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 85px;"  class="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Discount</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 105px;" class="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Subtotal</th>
                                        <th style="position: sticky; top: 0; z-index: 20; min-width: 150px;" class="px-3 py-2 text-left text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-700">Warehouse</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-neutral-800 divide-y divide-gray-100 dark:divide-neutral-700">
                                    @foreach ($order->orderItems as $index => $orderItem)
                                        <tr style="height: 40px;">
                                            <td style="position: sticky; left: 0; z-index: 10; min-width: 140px;" class="px-3 text-[11px] font-medium text-gray-900 dark:text-gray-100 whitespace-nowrap bg-white dark:bg-neutral-800 border-r border-gray-200 dark:border-neutral-700">
                                                {{ $orderItem->item->item_code }}
                                            </td>
                                            <td style="min-width: 500px;" class="px-3 text-[11px] text-gray-700 dark:text-gray-300 whitespace-nowrap">
                                            ...
                                            <td class="px-3 text-[11px]">
                                                <select
                                                    ...
                                                    @disabled($order->oracle_at || auth()->user()->isSalesHead())
            ...
            <div
                ...
                @php
                    $isCancelled = $order->order_status === \App\Enums\OrderStatusEnum::CANCELED;
                @endphp
                @if ($isCancelled)
                    <span class="text-sm text-red-600 dark:text-red-400 italic">
                        {{ __('Order cancelled — push to Oracle is disabled.') }}
                    </span>
                @elseif (!$order->oracle_at && !auth()->user()->isSalesHead())
                ...
                @elseif ($order->oracle_at)
                    <span class="text-sm text-gray-700 dark:text-gray-300">
                        {{ __('Order Already Entered to Oracle') }}
                    </span>
                @else
                    <span class="text-sm text-gray-500 dark:text-gray-400 italic">
                        {{ __('View only — push to Oracle is restricted.') }}
                    </span>
                @endif
            </div>
        </x-modal>
                ...
                <div class="mt-6 space-y-6">
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="flex items-end ml-6">
                            <div x-data x-show="{{ auth()->user()->isAdmin() || auth()->user()->isSupplyChain() || auth()->user()->isCmdKhi() || auth()->user()->isCmdLhr() || auth()->user()->isScmLhr() || auth()->user()->isSalesHead() ? 'true' : 'false' }}" class="text-left me-4">
