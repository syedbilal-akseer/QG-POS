...
class CustomerReceiptController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $receipt = DB::transaction(function () use ($validated, $customer, $receiptNumber, $currentYear, $outstanding, $receiptType, $request) {
            ...
            $this->logActivity('Receipts', 'create', "Mobile App: Receipt #{$receipt->receipt_number} created for customer {$customer->customer_name}.", [
        ...
        // OUTSIDE the transaction so a notification failure can't roll back the receipt.
    ...
    public function enterToOracle(Request $request, $receiptId): JsonResponse
    {
        ...
        try {
            ...
            $this->logActivity('Receipts', 'push', "Mobile App: Receipt #{$receipt->receipt_number} pushed to Oracle.", [
                'receipt_id' => $receipt->id,
                'oracle_receipt_number' => $oracleReceipt->receipt_number
            ]);
    ...
    public function receiptHistory(Request $request): JsonResponse
    {
        ...
        //
        // Filter expression: COALESCE(DATE(receipt_date), DATE(created_at))
        //   - If the receipt has a business-facing `receipt_date` (the date the
        //     user entered when creating it) → match against that date.
        //   - Otherwise fall back to the insert timestamp's date so older
        //     receipts where receipt_date was never set still show up.
        //
        // Previously this filtered ONLY on created_at, which surprised users
        // when (for example) a receipt entered on 18 Apr for receipt_date
        // 25 Apr wouldn't appear in a "25 Apr" query.
        $parseDate = function (?string $raw): ?string {
            if (!$raw) return null;
            // Try d-m-Y first (the mobile UI sends this); fall back to Y-m-d.
            try {
        ...
        $startDate = $request->filled('start_date')
            ...
            : ($request->filled('date_from') ? $parseDate($request->date_from) : null);

        $endDate = $request->filled('end_date')
            ? $parseDate($request->end_date)
            : ($request->filled('date_to') ? $parseDate($request->date_to) : null);

        if ($startDate) {
            $query->whereRaw(
                'COALESCE(DATE(receipt_date), DATE(created_at)) >= ?',
                [$startDate]
            );
        }
        if ($endDate) {
    ...
    /**
     * Cancel a receipt with a mandatory reason. Receipts that have already
     * been entered to Oracle cannot be cancelled from the mobile app —
     * those must be handled by an admin / cmd user on the portal.
     *
     * POST /api/customer-receipts/cancel
     * Body: { "receipt_number": "2026-0139", "reason": "<text>" }
     */
    public function cancel(Request $request): JsonResponse
    {
        ...
        $receipt = CustomerReceipt::where('receipt_number', $validated['receipt_number'])->first();
        if (!$receipt) {
            return response()->json([
        ...
        if ($receipt->oracle_entered_at !== null) {
            return response()->json([
                'success' => false, 'status' => 400,
                'message' => 'This receipt has already been pushed to Oracle and cannot be cancelled here.',
            ], 400);
        }

        // NOTE: don't touch oracle_status here — that column is an ENUM
        // constrained to ('pending','entered','failed') and MySQL rejects
        // 'cancelled' as a truncation error. cancelled_at is the source of
        // truth for cancellations; oracle_status only describes the Oracle
        // push state and is left as-is.
        $receipt->update([
        ...
        // Notify creator + admin / cmd-* users in the receipt's OU.
        try {
            app(\App\Services\OrderReceiptNotifier::class)->receiptCancelled($receipt, $validated['reason']);
        } catch (\Throwable $e) {
        ...
        return response()->json([
            ...
            'message' => 'Receipt has been cancelled.',
            'data'    => [
