...
{
    /**
     * Inline guard — Sales-head is view-only. (Laravel 11 removed the
     * Controller::middleware() helper, so we call this from each protected action.)
     */
    ...
    /**
     * Block users explicitly excluded from receipts (orders-only leads).
     */
    private function blockIfReceiptsForbidden(): void
    {
        if (auth()->check() && !auth()->user()->canAccessReceipts()) {
    ...
    public function edit($id)
    {
        ...
        // IMPORTANT: keep receipt_method, receipt_method_id and iban_number in
        // the select. The Alpine bank picker filters instruments by
        // receipt_method, matches the previously-saved cheque by
        // receipt_method_id (cheque rows store both instrument_id +
        // receipt_method_id), and displays iban_number in the picked-account
        // card. Dropping any of these silently broke pre-selection: the picker
        // failed to find the existing instrument, selAcc stayed null, and on
        // save the form posted an empty instrument_id — which made the
        // "Enter to Oracle" button stay disabled because $chk->instrument_id
        // was empty.
    ...
    public function update(\Illuminate\Http\Request $request, $id)
    {
        ...
        return redirect()->route('reciepts.edit', $receipt->id)->with('success', 'Receipt updated successfully!');
    }
    ...
    {
        // Receipt delete is admin-only. CMD-KHI, CMD-LHR, sales-head, and any
        // other role that reaches this route gets blocked here even though
        // the UI button is also hidden — defence in depth against URL spoofing
        // or cached pages.
        if (!auth()->user()->isAdmin()) {
            return redirect()->back()->with('error', 'Only admin users can delete receipts.');
        }

        $receipt = CustomerReceipt::findOrFail($id);

        if ($receipt->oracle_entered_at) {
            return redirect()->back()->with('error', 'Cannot delete a receipt that has already been pushed to Oracle.');
        }
    ...
    public function enterToOracle(Request $request, $id)
    {
        ...
        try {
            ...
            // Resolve bank_account_id: form now sends cash_bank_account_id for cash receipts.
            // Fall back to the existing receipt value if neither is submitted.
            $resolvedBankAccountId = $validated['cash_bank_account_id']
            ...
            // Handle multiple cheques update/creation if present in the enterToOracle request
            if ($request->has('cheques') && is_array($request->cheques)) {
                // Delete existing cheques
                $receipt->cheques()->delete();
                ...
                foreach ($request->cheques as $index => $chequeData) {
                    ...
                    \App\Models\ReceiptCheque::create([
                        ...
                        'cheque_amount' => $chequeData['cheque_amount'] ?? 0,
                        'cheque_date' => $chequeData['cheque_date'] ?? null,
                        'reference' => $chequeData['reference'] ?? null,
                        'comments' => $chequeData['comments'] ?? null,
                        'is_third_party_cheque' => isset($chequeData['is_third_party_cheque']) ? (bool)$chequeData['is_third_party_cheque'] : false,
                        'maturity_date' => $chequeData['maturity_date'] ?? null,
                        'cheque_images' => [], // Skip image uploading in ajax for now unless strictly needed
                        'status' => 'pending',
                    ]);
                }
            ...
            $explicitReceiptMethodId = $request->cash_receipt_method_id ?? ($request->cheques[0]['receipt_method_id'] ?? null);
            ...
            // Push notification to the salesperson who created the receipt
            app(\App\Services\OrderReceiptNotifier::class)->receiptPushedToOracle($receipt);
