...
class PriceListController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            ActivityLog::create([
                ...
                'action' => 'create',
                'module' => 'Price Lists',
                'description' => "Uploaded price list '{$file->getClientOriginalName()}' ({$stats['total']} rows processed)",
    ...
    public function updatePrice(Request $request, ItemPrice $price)
    {
        ...
        if ($oldPrice != $newPrice) {
            ...
            ActivityLog::create([
                ...
                'action' => 'update',
                'module' => 'Price Lists',
                'description' => "Updated price for {$price->item_code} from {$oldPrice} to {$newPrice}",
                'ip_address' => request()->ip(),
            ]);
    ...
    private function prepareComparisonData(int $uploadId): array
    {
        ...
        foreach ($processedItems as $item) {
            ...
            // Decide whether this row is genuinely brand-new in MySQL, or just
            // an existing row whose previous_price column happens to be null
            // (e.g. because the corruption cleanup wiped it). created_at being
            // within the upload window means the row was inserted by THIS
            // import — that's a real "new" item; everything else is an
            // existing item with prior history we just don't have on file.
            $isReallyNew = $item->created_at
                && $item->created_at->between($startTime, $endTime);
            ...
            if ($oldPrice && $oldPrice > 0) {
                ...
                // Existing MySQL row, no prior price recorded → treat as "same"
                // so it doesn't show up as a spurious "new" item.
                $priceStatus     = 'same';
                $backgroundColor = '#f8f9fa';
            }
    ...
    private function enrichWithOracleIds(array $excelData): array
    {
        ...
        foreach ($excelData as $item) {
            $itemCode = $item['item_code'];
            // Strip NBSP (U+00A0) and trim whitespace from UOM before any
            // Oracle lookup. Some Excel exports put NBSP at the end of UOM
            // cells (e.g. "DRM ", "DZN ") and that byte slips
            // past plain trim(). Oracle's price_list_view stores UOMs
            // cleanly, so a polluted UOM produces a false NO_RECORD_FOUND
            // and the price gets pushed as "create new" even though the
            // existing Oracle row is right there under the clean UOM.
            $uom = trim(str_replace("\xC2\xA0", ' ', (string) ($item['uom'] ?? '')));
            ...
            try {
                ...
                // Try every plausible spelling — bare, no-spaces, and with-spaces.
                // MySQL stores both "Karachi-Wholesale" and "Karachi - Wholesale"
                // for the same Oracle list. Skipping a variant here causes a
                // duplicate insert into qg_price_list_enddate instead of an
                // end-date on the existing Oracle price.
                $oraclePrice = null;
                foreach ($this->priceListNameVariants($priceListName) as $variant) {
                    $oraclePrice = DB::connection('oracle')
                        ->table('apps.qg_price_list_view')
                        ->select(['list_line_id', 'inventory_item_id', 'price_list_id', 'list_price as current_price'])
                        ->where('item_code', $itemCode)
                        ->where('uom', $uom)
                        ->where('price_list_name', $variant)
                        ->first();

                    if ($oraclePrice) {
                        if ($variant !== $priceListName) {
                            Log::info('Matched Oracle price under name variant', [
                ...
                // Try every name variant so the lookup matches regardless of
                // how the row was spelled in MySQL.
                $finalListHeaderId = null;
                try {
                    $priceListHeader = null;
                    foreach ($this->priceListNameVariants($priceListName) as $variant) {
                    ...
                    // IMPORTANT: Karachi Wholesale = 7011 (NOT 7007 — 7007 is
                    // Lahore Corporate). Both `Karachi - Wholesale` (with
                    // spaces around the dash) and `Karachi-Wholesale` (no
                    // spaces) appear in the data so both keys are mapped.
                    ...
                    // Use fallback mapping on exception — same correct values
                    // as the fallback block above.
                    $priceListMapping = [
                        'Karachi - Corporate'   => '7010',
                        'Karachi - Trade Price' => '7012',
                        'Karachi - Wholesale'   => '7011',
    ...
    public function enterNewPricesToOracle(Request $request)
    {
        ...
        try {
            ...
            // Verified against the data: 7011 = Karachi Wholesale (both spelling
            // variants), 7007 = Lahore Corporate, 7008 = Lahore Wholesale,
            // 7009 = Lahore Trade Price. The previous version had
            // `Karachi - Wholesale => 7007` (Lahore Corporate's id) which is
            // why Karachi Wholesale prices were silently being pushed against
            // the wrong Oracle price list, or rejected for failing constraints.
            $priceListMapping = [
                'Karachi - Corporate'   => '7010',
                'Karachi - Trade Price' => '7012',
                'Karachi - Wholesale'   => '7011',
            ...
            foreach ($oracleEnrichedData as $index => $item) {
                ...
                // Flush in batches to avoid memory issues and timeouts.
                // BUG FIX: the previous gate only checked $endDateBatch's size,
                // so when every item in an upload routed to the "create new"
                // branch (oracle_found = false for all of them) the
                // $newPriceBatch never reached the flush call and was silently
                // dropped. The controller still logged "X created" because the
                // results array was populated regardless. Now each batch is
                // flushed independently when it reaches the threshold.
                if (count($endDateBatch) >= $batchSize || count($newPriceBatch) >= $batchSize) {
                    try {
                        if (!empty($endDateBatch)) {
                            DB::connection('oracle')->table('apps.qg_price_list_enddate')->insert($endDateBatch);
                        }
                        if (!empty($newPriceBatch)) {
            ...
            // Flush any remaining items — gate on EITHER batch being non-empty,
            // not just $endDateBatch (see comment above).
            if (!empty($endDateBatch) || !empty($newPriceBatch)) {
                try {
                    if (!empty($endDateBatch)) {
                        DB::connection('oracle')->table('apps.qg_price_list_enddate')->insert($endDateBatch);
                    }
    ...
    /**
     * Return every plausible spelling of a price-list name so Oracle lookups
     * don't miss the existing record. MySQL has been inconsistent over time —
     * some rows are stored "Karachi - Wholesale" (with spaces), some
     * "Karachi-Wholesale" (without). Trying only one variant lets the import
     * silently create a duplicate price in Oracle instead of end-dating the
     * existing one.
     */
    private function priceListNameVariants(string $priceListName): array
    {
        $name      = trim($priceListName);
        $noSpaces  = preg_replace('/\s*-\s*/', '-',  $name);
        $withSpaces = preg_replace('/\s*-\s*/', ' - ', $name);

        $variants = [$name];
        foreach ([$noSpaces, $withSpaces] as $variant) {
            if ($variant !== null && $variant !== '' && !in_array($variant, $variants, true)) {
                $variants[] = $variant;
            }
        }
        return $variants;
