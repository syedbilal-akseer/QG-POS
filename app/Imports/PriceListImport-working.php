    ...
    SkipsOnFailure
{
    ...
    protected function processSinglePrice(array $itemInfo, string $priceListName, array $priceListMapping, float $newPrice): void
    {
        ...
        try {
            // Validate against MySQL item_prices table first
            $existingItem = $this->validateOracleItem($itemCode, $uom);
            if (!$existingItem) {
                Log::warning('Item not found in item_prices table', [
                    'item_code' => $itemCode,
                    'uom' => $uom,
    ...
    /**
     * Validate item exists in MySQL item_prices table and return item details.
     */
    protected function validateOracleItem(string $itemCode, string $uom): ?object
    {
        try {
            ...
            Log::error('MySQL item validation failed', [
                'item_code' => $itemCode,
                'uom' => $uom,
                'error' => $e->getMessage()
            ]);
