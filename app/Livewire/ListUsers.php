...
class ListUsers extends Component implements HasForms, HasTable
{
    ...
    /** Additional roles (multi-select). Existing primary `role` is unchanged. */
    ...
    public function createUser()
    {
        $this->validate([
            ...
            'new_role' => 'required|string|in:admin,user,supply-chain,sales-head,price-uploads,cmd-khi,cmd-lhr,cmd-head,scm-lhr,hod,line-manager,account-user,invoice-manager,audit',
        ...
        $user = User::create([
            ...
            'role' => $this->new_role,
            'additional_roles' => !empty($this->new_additional_roles)
                ? array_values(array_filter($this->new_additional_roles))
                : null,
    ...
    public function openEditModal(User $user)
    {
        ...
        $this->role = $user->role;
        $this->additional_roles = $user->getAdditionalRoles();
    ...
    public function updateUser()
    {
        $this->validate([
            ...
            'role' => 'required|string|in:admin,user,supply-chain,sales-head,price-uploads,cmd-khi,cmd-lhr,cmd-head,scm-lhr,hod,line-manager,account-user,inventory-manager,invoice-manager,audit',
        ...
        $data = [
            ...
            'email' => $this->email,
            'role' => $this->role,
            'additional_roles' => !empty($this->additional_roles)
                ? array_values(array_filter($this->additional_roles))
                : null,
    ...
    public function render(): View
    {
        return view('livewire.list-users', [
            ...
            'availableAdditionalRoles' => $this->getAvailableAdditionalRoles(),
        ]);
    }
    ...
    /**
     * Additional roles that can be assigned ON TOP of the primary role.
     * Currently view-only roles for scoped read access.
     */
    public function getAvailableAdditionalRoles()
    {
        return [
            'view-khi' => 'View Karachi data (read-only)',
            'view-lhr' => 'View Lahore data (read-only)',
            'view-all' => 'View All data (read-only)',
        ];
    }
