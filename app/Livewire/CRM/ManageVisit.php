...
class ManageVisit extends Component implements HasTable, HasForms
{
    ...
    {
        return $table
            ->query(MonthlyVisitReport::where('salesperson_id', auth()->user()->salesperson_id))
            ->columns([
            ...
            ->actions([ 
                Tables\Actions\Action::make('view')
                    ...
                    ->action(function (MonthlyVisitReport $record) {
                        $this->reset();
                        $this->redirectRoute('monthlyTourPlans.visits', ['monthlyVisitReport' => $record], navigate: true);
                    })
                    ->color('violet'),
            ])
    ...
    {
        return view('livewire.crm.manage-visit');
    }
}
