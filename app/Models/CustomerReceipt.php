...
class CustomerReceipt extends Model
{
    ...
    /**
     * Always include a lifecycle status (`receipt_status`) when the model is
     * serialized to JSON. Mirrors how the orders API exposes `order_status` —
     * any endpoint that returns this model now ships the status for free.
     *
     *   pending   — created, not yet pushed to Oracle and not cancelled.
     *   pushed    — successfully pushed to Oracle (oracle_entered_at set).
     *   cancelled — soft-cancelled via the cancel endpoint.
     */
    ...
    public function getReceiptStatusAttribute(): string
    {
        ...
        if ($this->oracle_entered_at !== null) {
            return 'pushed';
        }
        return 'pending';
    }
