...
class SyncOracleUsers extends Command
{
    ...
    private function syncShippingUsers(int &$syncedUsers, int &$syncedOrganizations)
    {
        ...
        $protectedRoles = \App\Models\Role::pluck('name')->toArray();
        ...
        foreach ($oracleUsers as $oracleUser) {
            ...
            if ($existingUser) {
                $currentRole = $existingUser->getRoleName();
                ...
                if (in_array($currentRole, $protectedRoles)) {
                    $this->line("Updated shipping user (preserved role '{$currentRole}'): {$user->name} -> Oracle ID: {$oracleUser->user_id}");
                } else {
    ...
    private function syncAllUsers(int &$syncedUsers, int &$syncedOrganizations)
    {
        ...
        foreach ($oracleUsers as $oracleUser) {
            ...
            if ($existingUser) {
                ...
                if (in_array($currentRole, $protectedRoles)) {
                    $this->line("Updated salesperson (preserved role '{$currentRole}'): {$user->name} -> Oracle Name: {$oracleUser->user_name}");
                } else {
